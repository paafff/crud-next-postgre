...
const EditProduct = ({
  ...
  const [name, setName] = useState(propsProducts.name);
  const [brand, setBrand] = useState(propsProducts.brandId);
  const [price, setPrice] = useState(propsProducts.price);
  ...
  const handleEdit = async (e: SyntheticEvent) => {
    ...
    await axios.patch(`/api/products/${propsProducts.id}`, {
      name: name,
      price: Number(price),
